...
/** Per-plugin V8 heap cap (MB). Hardening: bounds a runaway plugin's memory. */
const PLUGIN_MEM_MB = Number(process.env.PLUGIN_MEM_MB ?? 64);
...
export function spawnPlugin(
  ...
  // Check if Node supports --permission (v22+)
  const nodeMajor = Number(process.versions.node.split(".")[0]);
  const hasPermissions = nodeMajor >= 22;

  const args: string[] = [];
  if (hasPermissions) {
    // Node 22+ permission model
    args.push("--permission");
    args.push(`--allow-fs-read=${pluginDir}`);
    args.push(`--allow-fs-read=${join(pluginDir, "..", "..")}`);
    if (grantedCaps.includes("exec")) {
      args.push("--allow-child-process");
    }
  }
  // Resource hardening (all tiers): cap the plugin's V8 heap.
  args.push(`--max-old-space-size=${PLUGIN_MEM_MB}`);
  // Node < 22: no permission flags — capability gating via IPC only

  const capsJson = JSON.stringify(grantedCaps);
  const child = spawn(NODE_BIN, [...args, RUNNER, pluginPath, capsJson], {
