...
$ModelProfiles = [ordered]@{
    ...
    #      ngl 99→117, threads 8→13, batch 6637, ubatch 2875
    #      OverrideTensor offloads ALL ffn expert tensors to CPU (MoE weight trick)
    #      Bench result (no MTP, no KV quant): pp256 +4.4%, tg128 +1.0% vs baseline
    #      Production tg with MTP+KV quant will be higher than raw bench numbers.
    #      64k profile: same expert offload applied; monitor VRAM — may need ngl tuned.
    ...
    'Qwen3.6 27B · MTP  [32k · default]' = @{
        ...
        BatchSize        = 6637
        UBatchSize       = 2875
        OverrideTensor   = 'blk\.\d+\.ffn_.*_exps\.=CPU'
    ...
    #
    #    llama-optimus tuning (2026-05-26, build bb28c1fe2, RTX 4090):
    #      ngl 99→129, threads 8→7, batch 6377, ubatch 4399
    #      OverrideTensor offloads ODD-numbered block experts to CPU only
    #      (blk\.(?:[0-9]*[13579])\.ffn — more conservative than Q5 full offload)
    #      Bench result (no MTP, no KV quant): pp256 +3.7%, tg128 +1.0% vs baseline
    ...
    'Qwen3.6 27B · Q4_K_M + MTP  [32k]' = @{
        ...
        BatchSize        = 6377
        UBatchSize       = 4399
        OverrideTensor   = 'blk\.(?:[0-9]*[13579])\.ffn_.*_exps\.=CPU'
        ReasoningBudget  = '3072'
        MaxPredictTokens = '8192'
        SpecType         = 'draft-mtp'
        SpecDraftNMax    = 3
...
Write-Host "  ${c}║   ╚══════╝╚══════╝╚══════╝   ╚══════╝   ╚═╝          ║${r}"
Write-Host "  ${c}║                                                      ║${r}"
Write-Host "  ${c}║              Stack Launcher  v1.064                   ║${r}"
Write-Host "  ${c}╚══════════════════════════════════════════════════════╝${r}"
Write-Host ""
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAXPiTPGTuUMAJv
# 2ZS/woFBGlXMIdzzF55ql1DgSqnbwqCCAxgwggMUMIIB/KADAgECAhAnjvKeW2tW
...
# MAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIBAiJ+vrw+0nculFw+y/v52L
# UXjFxigIxxYjcpsjFfTOMA0GCSqGSIb3DQEBAQUABIIBABjIkhSgSMZCjGO6jzte
# s9b6r6sYYJuFXlZB8VC5ZtVntHzHEpNHKS5lI+D2x/L7TXTk+cag0Uuthu3XczJJ
# sk6JxhBWw9AyqMLcCJZGOldJ0FirYvnL5ZGxBuo8PvyG0sqp2uxKfrK9JV1AjOaD
# IuFGggLE1uMYV4r5YGVSQ0Ag1KnbF9d1Cd3grgB/iZfa9LyW8M1LQJ84wdKGcvfN
# qjNylILu5n1wDbk1pXvbcKcrWzyTWhLmrPcH6XP5V90nlAdxeVB3VKoK+dugfY1A
# sY+IodCCr7bzyG8YybQvnkv5EWsL0JJg9So0t8yyxihMhLvZp7f4shDwtmonnt7H
# YCE=
