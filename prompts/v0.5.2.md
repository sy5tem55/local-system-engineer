...
> Fixes sudo_delegation_block loop. Removes conflicting OUTPUT RULES and SUDO DELEGATION FORMAT.
> Adds KB path reference.
>
> Requires: tool v1.5.5 · filter v1.1.0 · context-monitor v1.0.0
>
> Changes from v0.5.1:
>   • TOOLS: sudo_delegation_block one-liner: added "DO NOT call again."
>   • Removed SUDO DELEGATION FORMAT section — format is owned by the tool, not the prompt
>   • KNOWLEDGE BASE: added KB path (/opt/local-se/kb/)
>   • (tool v1.5.5) sudo_delegation_block: added RETURN VALUE SEMANTICS block
>   • (tool v1.5.5) read_file: PRIVILEGED PATH BEHAVIOUR now explicitly prohibits cat/python/base64 workarounds
...
ENVIRONMENT
───────────
Version:  v0.5.2 (tool v1.5.5 · filter v1.1.0 · context-monitor v1.0.0)
