...
$ModelProfiles = [ordered]@{
    ...
    'Qwen3.6 27B · MTP  [64k · q8_0]' = @{
        # VRAM NOTE: Q5_K_M weights (~19.4 GB) + 64k q8_0 KV (~2.15 GB) + overhead (~1.5 GB)
        # = ~23 GB total, leaving only ~1 GB headroom on RTX 4090 (24 GB).
        # KV quants below q8_0 either cause language drift (q4_0, iq4_nl) or don't save
        # enough to fix the headroom (q5_1 saves ~0.7 GB — still spills under MTP load).
        # Use Q4_K_M [64k · q8_0] profile if you need reliable 64k + MTP on this hardware.
        ModelFile        = 'Qwen3.6-27B-Q5_K_M.gguf'
        CtxSize          = 65536
        GpuLayers        = 117
        FlashAttn        = $true
...
Write-Host "  ${c}║   ╚══════╝╚══════╝╚══════╝   ╚══════╝   ╚═╝          ║${r}"
Write-Host "  ${c}║                                                      ║${r}"
Write-Host "  ${c}║              Stack Launcher  v1.070                   ║${r}"
Write-Host "  ${c}╚══════════════════════════════════════════════════════╝${r}"
Write-Host ""
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCM+msnfRqUibAI
# OEORin0PiuRhlaBbK0ypFAYaNs8hnKCCAxgwggMUMIIB/KADAgECAhAnjvKeW2tW
...
# MAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIBsESiL3DxVFipyysm6scapz
# 2LAVTl04Euu1BhMANRKoMA0GCSqGSIb3DQEBAQUABIIBAA/ugcs+xe31usC3nyou
# nonpLo2Ws02wzXVbC2tBdVgPtZkLGQK0cQs7nDuGOvPOgj+rFNOySraEzvLggGTi
# R3/1UTV3fdyMiO20lYRM2Os0jeEL0Oy3uPQklyPB7tLVF4Tr7UkRbuik/MTKYuem
# 6RWO1nDemnOnNzg+dPsuudsZUVftkFXU9HVfYAUMuNYY1po06XkkhXWDe79UDFnN
# twTKY7fIv34/fnxnorwda6LUVWkdilHx8p0WbnwOOUKrLeAS8L2owlvcxUbTWpxT
# zRFHWp2y2HM3gq2umi5O9HAqWtbm/vHMBFy5T1rGNCWyDa56NHIcDZx3jphVcAh/
# GQQ=
