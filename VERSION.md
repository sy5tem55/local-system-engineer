...
> Last updated: 2026-07-18 (Cowork) — P0-1 registry reconcile: live-verified goethe.py v0.4.0-a (7238 lines, raw `ba0c1a2015c89098…`), goethe_mcp `__version__` 1.11.1 (815 lines, raw `1d903a29ca63ec50…`) — VERSION.md's "v1.9.3" was stale. llama-server build identity RECONCILED per node (both a6647b1 and b9577 are obsolete — rebuilt since): LUCIFER `/home/sy5/llama.cpp/build/bin/llama-server` = version 20 (`bf2c86ddc`); node3090 `/opt/llama.cpp/bin/llama-server` = version 64 (`e8f19cc0a`), both GNU 14.2.0 Linux x86_64. Gateway runs from the repo checkout (`/home/sy5/projects/local-system-engineer/tools/`), loading vaultwarden v1.3.0 + pfsense v1.0.0 + net_discovery v1.0.0 via `--also`.
> Previous: 2026-07-15 (Codex) — TRAUM production reconciliation: dream tools v0.12.0/v0.4.0/v0.3.0; Qwen 1024-dimensional KB contract
...
|---|---|---|---|
| Tool | **Goethe v0.4.1** | 2026-07-18 | ✅ **DEPLOYED** — PH5-1/PH5-2 refactor: `TrustPolicy` + KB surface (14 methods) extracted to **`tools/goethe_kb.py`** (`KBMixin`, 1435L, raw `82d4201dd69ddf2d…`); goethe.py 7238 → 5851L, raw `47d9fd659615d6a1…`. MCP tool-list diff empty (38 tools); contract tests 420/420. Previous — v0.4.0-a: TRAUM gateway integration; production embeddings are `qwen3-embedding:0.6b` (1024 dimensions). Earlier lineage: `hermes_plan` → `planner` rename + `<think>`-strip before JSON envelope extraction; 5819 lines, raw `d3ccccf1…` (verified live 2026-07-02 via MCP bridge). Δ v0.2.5→v0.2.9: +476 lines. **Recent chain:** v0.2.6 SSH overhaul (`ssh_run` argv + `ssh_script` scp + ControlMaster + complexity guard — 3 exit-255 root causes) → v0.2.7 **HERMES RETIRED** (`_call_hermes`/`_kanban_create_card` stubbed; `_call_node_planner` 2-path cascade + NODE3090_* valves) → v0.2.8 planner PATH-3 VRAM-gated local Gemma GGUF spawn (E4B/26B-A4B/31B, vision mmproj; PLANNER_* valves) → v0.2.9 planner rename + think-tag JSON fix. Previous: v0.2.5 (5343 lines, raw `aa2aa1e1…`). **Lineage:** v0.1.0 (P31, Cogitator fork, 4721 lines) → v0.2.0 (download-monitor bugfix + audit pass) → v0.2.1 (KB doc-id resolution, stable goethe.py filename, `_resolve_kb_id`) → v0.2.2 (3 ground-truth-before-action rules in execute_command) → v0.2.3 (wake_node ping-first + KB-first) → v0.2.4 (shutdown_node two-step gate) → v0.2.5 (fetch_url reddit/camoufox Firecrawl fallback). **Cogitator v1.7.24 superseded** — staged only, never deployed to MCP; OWUI retired. |
| MCP Gateway | **goethe_mcp v1.11.1 / launcher v2.1.3** | 2026-07-15 | ✅ deployed — live-verified 2026-07-18: `__version__ = "1.11.1"`, 815 lines, raw `1d903a29ca63ec50…` (registry previously said v1.9.3 — stale; v1.10.x–v1.11.x added TRAUM episode journaling + redaction sweep). `_TokenGuard` accepts — `_TokenGuard` accepts `Bearer <token>` or raw token; port 9700; HTTP transport; canonical LUCIFER launcher is detached/SIGHUP-proof and verifies the bound process before writing its PID. |
...
| System Prompt (LUCIFER/node4090) | **v0.6.0** | 2026-07-04 | ✅ canonical per P0-5 — `prompts/v0.6.0.md` (REQUEST-SHAPE MAPPINGS, tasks.db ledger MULTI-BLOCK rule, planner v2 / kb_verify / time_check / run_tests / assert_state entries, TIME DISCIPLINE, ledger-first HANDOVER). v0.5.16–19 purged to `.backups/pre-purge-20260718/` (P0-3, 2026-07-18); canonical dir = `prompts/`. |
| System Prompt (node3090) | **v0.3.0** | 2026-07-18 | ⏳ ready to paste — `prompts/node3090-v0.3.0.md`, synced to Goethe v0.4.4 / goethe_mcp v1.11.2 / 37 tools: adds kb_verify/time_check/run_tests/assert_state/mentor_demote + REQUEST-SHAPE MAPPINGS, drops search_rfc, fixes index_to_kb signature (origin=), strips the hardcoded GOETHE_MCP_TOKEN (SEC) + hardens the relaunch block to 127.0.0.1/scoped CORS. Previous: v0.2.1 (deployed since Goethe v0.2.9 era — operator-confirmed stale 2026-07-18), v0.1.0 (2026-06-29). |
...
| Component | Version | Shipped | Status |
|---|---|---|---|
| Dream Runner | **dream_runner.py v0.12.0** | 2026-07-15 | ✅ **PRODUCTION CODE** — five registered passes; secret redaction; pass-scoped artifacts; lock/session guards; LLM and wall-clock budgets; no-dream-of-dreams assertion; crash records route to `lse-errors-1024`; loaded-model-aware node3090 routing. Ledger mining is the reviewed one-off artifact in `kb/ledger-mining-proposals.md`, not a generic nightly pass. |
| Dream Apply | **dream_apply.py v0.4.0** | 2026-07-15 | ✅ **PRODUCTION CODE** — human-gated allowlisted dispatch; oldest-first multi-day queue with 14-day expiry; prompt rules remain human-merge-only; no Elasticsearch delete primitive. The dormant auto-apply allowlist remains empty after the LOSS verdict. |
| Dream Digest | **dream_digest.py v0.3.0** | 2026-07-15 | ✅ **PRODUCTION CODE** — ≤30-line digest; pass-scoped/legacy artifact compatibility; failed-night escalation; pending proposal type breakdown; 1024-dimensional error-index corpus stats. |
| Systemd Timer | **goethe-dream.timer** | 2026-07-15 | ✅ **ACTIVE / RECONCILED** — repository units installed; authoritative checkout; fail-visible five-pass cycle; 45-minute outer timeout; 03:30 + ≤15m jitter; manual acceptance start returned success with the active-session guard correctly engaged. |
...
|---|---|---|---|
| `goethe.py` (v0.4.0-a, live 2026-07-18) | 7238 | `ba0c1a2015c89098…` | _n/a (llama-ui does not reformat; raw sha is authoritative)_ |
| `goethe_mcp.py` (v1.11.1, live 2026-07-18) | 815 | `1d903a29ca63ec50…` | _n/a_ |
