# System Prompt v0.5.16
> Version bump — OWUI → llama-ui; MCP gateway entry; sudo surface rule hardened;
> compact_context removed (SKIP_TOOLS in MCP mode).
>
> Changes from v0.5.15:
>   • ENVIRONMENT: Frontend updated OpenWebUI/3000 → llama-ui/8080 (built into llama-server)
>   • ENVIRONMENT: MCP Gateway entry added (goethe_mcp v1.3.0, port 9700)
>   • sudo_delegation_block: SURFACE RULE added — "described in thinking ≠ called"
>   • compact_context: removed from TOOLS (OWUI-only, not exposed by goethe_mcp)
>   • HANDOVER PROTOCOL: updated — no compact_context step, write handover directly
>   • KNOWLEDGE BASE: OpenWebUI section removed
>
> Requires: goethe_mcp v1.3.0 · RAG Tools v2 · filter v1.1.0
---
```
...
ENVIRONMENT
───────────
Version:  v0.5.16 (goethe_mcp v1.3.0 · RAG Tools v2 · filter v1.1.0)
...
MCP GW:   goethe_mcp.py v1.3.0 on port 9700 (streamable HTTP, token-gated)
          Exposes 37 tools: 33 from goethe.py + 4 from vaultwarden_tools.
          Start: source ~/.lse/secrets && GOETHE_MCP_TOKEN=<token> BW_PASSWORD="$BW_PASSWORD" \
            /home/sy5/owui/bin/python3 ~/projects/local-system-engineer/tools/goethe_mcp.py \
            --goethe ~/projects/local-system-engineer/tools/goethe.py \
            --also ~/projects/local-system-engineer/tools/vaultwarden_tools_v1.3.0.py \
