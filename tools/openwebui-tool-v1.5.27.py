...
description: Safe shell execution for the Local System Engineer (LSE) WSL2/Ubuntu 24.04 agent.
  ...
  record_error, check_error_kb, record_outcome, mentor_correct, pfsense_query,
  start_node_agent, stop_node_agent, and search_reddit.
  ...
  Changelog:
    v1.5.27: search_web — DATE-SENSITIVE QUERIES rule: check actual date via
              ...
              Prevents stale KB hits being accepted for time-sensitive topics.
    v1.5.26: search_web — timeout=(5,10) to prevent connection hang on VPS hiccup.
              Strengthened NO YEAR INJECTION rule in docstring.
    v1.5.25: search_reddit() — Reddit search wrapper via SearxNG site:reddit.com operator.
              No OAuth, no API key, no Reddit account footprint. Routes through VPS SearxNG.
    v1.5.24: start_node_agent / stop_node_agent — on-demand llama-cpp server lifecycle via SSH.
              agent_profile added to _NODE_REGISTRY (model path, ctx, gpu_layers, quant flags).
    v1.5.23: _NODE_REGISTRY node3090 — agent_port 1234→8080, agent_type lmstudio→llama-cpp.
              Port 1234 is LM Studio; llama.cpp server defaults to 8080.
    v1.5.22: wake_node — corrected WoL endpoint to /api/v2/services/wake_on_lan/send
              (missing /send suffix caused HTTP 404). Added fast-fail guard: if
              pfSense returns ERROR or HTTP error, abort immediately instead of
              burning 120s on the ping poll.
...
class Tools:
    ...
    _BLOCKED_WRITE_FILENAMES = {
        ...
        ".cshrc", ".tcshrc", ".fishrc",
        ".ssh/authorized_keys", ".ssh/config", ".ssh/id_rsa", ".ssh/id_ed25519",
        ".gnupg/gpg.conf",
    ...
    def _is_allowed_write(self, path: str) -> bool:
        ...
        if basename in self._BLOCKED_WRITE_FILENAMES or rel_home in self._BLOCKED_WRITE_FILENAMES:
            return False
    ...
    def index_to_kb(
        ...
        content = content[:4000]  # hard cap: prevent token-heavy embeddings
        try:
    ...
    def pfsense_log_summary(
        ...
        """
        Fetch pfSense firewall logs and return a COMPACT STRUCTURED SUMMARY.

        ALWAYS use this function for firewall log analysis. NEVER call
        pfsense_query('/api/v2/status/logs/firewall') directly — that endpoint
        can return hundreds of kilobytes of raw log lines that fill context and
        trigger truncation, making subsequent analysis unreliable.

        This function:
          1. Fetches raw logs via the pfSense REST API (same auth as pfsense_query).
          2. Extracts statistics programmatically — no raw lines returned.
          3. Optionally enriches with an Ollama-generated anomaly narrative
             (fires only when OLLAMA_URL is reachable; skipped silently otherwise).

        RETURNED JSON keys (always present):
          log_count        — int: total entries in the time window
          window_hours     — int: hours covered
          top_blocked_ips  — list[{ip, count}]: top N source IPs by block frequency
          top_blocked_ports— list[{port, protocol, count}]: top N blocked dst ports
          interface_ratios — dict[iface, {pass_count, block_count, block_ratio}]
          anomalies        — str: Ollama narrative summary, or "" if unavailable

        Args:
            hours:   Time window to analyse (default 24). Applied as a filter if
                     the API supports it; otherwise the full log is fetched and
                     filtered locally by timestamp.
            top_n:   How many entries to return in top_blocked_ips /
                     top_blocked_ports (default 10).
            api_key: Vaultwarden-sourced key override (same semantics as
                     pfsense_query). Falls back to PFSENSE_API_KEY valve.
        """
        ...
        # ── 1. Fetch raw logs ─────────────────────────────────────────────────
        raw = self.pfsense_query(
            endpoint="/api/v2/status/logs/firewall",
            method="GET",
            api_key=api_key,
        )
        ...
        try:
            ...
            return f"ERROR: pfsense_log_summary — could not parse API response: {e}"

        # API may return {"data": [...]} or a bare list
        entries = data if isinstance(data, list) else data.get("data", [])
        if not entries:
            return _json.dumps({
                "log_count": 0, "window_hours": hours,
                "top_blocked_ips": [], "top_blocked_ports": [],
                "interface_ratios": {}, "anomalies": "",
            })
        ...
        def _parse_ts(entry):
            ...
            for fmt in ("%Y-%m-%dT%H:%M:%S%z", "%b %d %H:%M:%S"):
                try:
                    ...
                    continue
            return cutoff  # unknown format — include entry

        entries = [e for e in entries if _parse_ts(e) >= cutoff]

        # ── 3. Programmatic extraction ────────────────────────────────────────
        blocked_ips   = Counter()
        blocked_ports = Counter()   # key: (port, protocol)
        iface_pass    = Counter()
        iface_block   = Counter()

        for e in entries:
            act    = str(e.get("act",    e.get("action",    ""))).lower()
            src_ip = str(e.get("src",    e.get("src_ip",    "")))
            dst_p  = e.get("dstport",    e.get("dst_port",  e.get("dport", "")))
            proto  = str(e.get("proto",  e.get("protocol",  "tcp"))).lower()
            iface  = str(e.get("interface", e.get("iface",  "unknown")))

            if act in ("block", "reject"):
                if src_ip:
                    blocked_ips[src_ip] += 1
                if dst_p:
                    blocked_ports[(str(dst_p), proto)] += 1
                iface_block[iface] += 1
            else:
                iface_pass[iface] += 1
        ...
        top_ips = [
            {"ip": ip, "count": cnt}
            for ip, cnt in blocked_ips.most_common(top_n)
        ]
        top_ports = [
            {"port": int(p) if str(p).isdigit() else p, "protocol": pr, "count": cnt}
            for (p, pr), cnt in blocked_ports.most_common(top_n)
        ]
        all_ifaces = set(iface_pass) | set(iface_block)
        iface_ratios = {}
        for iface in all_ifaces:
            ...
            iface_ratios[iface] = {
                "pass_count":   p,
                "block_count":  b,
                "block_ratio":  round(b / (p + b), 4) if (p + b) > 0 else 0.0,
            }

        # ── 4. Ollama anomaly narrative (optional, non-blocking) ──────────────
        anomalies = ""
        try:
            import requests as _req
            ollama_url = getattr(self.valves, "OLLAMA_URL", "http://127.0.0.1:11434")
            # Build a compact context for the LLM — never the raw logs
            llm_context = (
                f"Firewall log summary ({len(entries)} entries, last {hours}h):\n"
                f"Top blocked IPs: {top_ips[:5]}\n"
                f"Top blocked ports: {top_ports[:5]}\n"
                f"Interface block ratios: { {k: v['block_ratio'] for k, v in iface_ratios.items()} }\n"
            )
            resp = _req.post(
                ...
                json={
                    "model": "llama3.2:3b",
                    "prompt": (
                        "You are a network security analyst. Given this firewall summary, "
                        "identify any anomalies or unusual patterns in 2-3 bullet points. "
                        "Be specific about IPs and ports. If everything looks normal, say so.\n\n"
                        + llm_context
                    ),
                    "stream": False,
            ...
            if resp.status_code == 200:
                anomalies = resp.json().get("response", "").strip()
        except Exception:
            pass  # Ollama unavailable — summary is still complete without narrative
        ...
        result = {
            ...
            "interface_ratios":  iface_ratios,
            "anomalies":         anomalies,
        }
        self._log(f"PFSENSE-LOG-SUMMARY: {len(entries)} entries, "
                  f"{len(top_ips)} blocked IPs, {len(top_ports)} blocked ports")
    ...
    def nmap_summary(
        ...
        cmd = ["nmap", "-sV", f"--top-ports={top_ports}", "-oX", "-", "--open"] + target_list
        cmd_str = " ".join(cmd)
                    ...
                    for port in info["ports"]:
                        if expected and port not in expected:
                            unexpected.append({
                                "ip":      ip,
                                "port":    port,
                                "service": info["services"].get(str(port), "unknown"),
                            })
            except _json.JSONDecodeError:
                unexpected = [{"error": "known_services JSON invalid — skipped baseline check"}]
        ...
        self._log(f"NMAP-SUMMARY: {len(scan_results)} hosts up, "
    ...
    def search_rfc(
        ...
        try:
            ...
            if resp.status_code == 404:
                return ("ERROR: lse-rfc-kb index not found. "
        ...
        self._log(f"RFC-SEARCH: '{symptom[:60]}' → {len(results)} results "
    ...
    def wake_node(self, node: str) -> str:
        ...
        self._log(f"WAKE-NODE: sending WoL for {node} ({reg['mac']}) on {reg['interface']}")
    ...
    def query_node_agent(
        ...
        payload: dict = {"messages": messages, "max_tokens": max_tokens, "temperature": 0.7}
        if model:
            payload["model"] = model
    ...
    def shutdown_node(self, node: str) -> str:
        ...
        SAFETY RULES — mandatory before calling:
          ...
          - Do NOT shut down node3090 if a ComfyUI or LM Studio job is running.
