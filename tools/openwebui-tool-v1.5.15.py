...
class Tools:
    ...
    def pfsense_query(
        ...
        SSL: pfSense uses a self-signed cert. verify=False — LAN-only, 192.168.1.57/32
          allowlisted on pfSense. Risk is acceptable.
        ...
        try:
            resp = requests.request(
                ...
                verify=False,   # self-signed cert, LAN-only — acceptable
                timeout=15,
            )
            try:
