...
BW_PASSWORD is configured via Open WebUI Valves (gear icon next to the tool).
It can also be overridden by the BW_PASSWORD environment variable.
It is never hardcoded here — change it in the Valves UI only.
...
class Tools:
    ...
    class Valves(BaseModel):
        BW_PASSWORD: str = Field(
            default="",
            description="Master password for the LSE Vaultwarden account (lse@local-se.ai). "
                        "Update here whenever the master password changes."
        )
    ...
    def vault_unlock(self) -> str:
        ...
        REQUIRED: BW_PASSWORD must be set via the Valves UI (gear icon next to the tool
        in Open WebUI) or the BW_PASSWORD environment variable. All other credentials
        are pre-configured and need no action.
        ...
        password = self.valves.BW_PASSWORD or os.environ.get("BW_PASSWORD", "")

        if not password:
            return "ERROR: BW_PASSWORD is not set. Open the tool's gear icon in Open WebUI and enter the master password in the Valves settings."
