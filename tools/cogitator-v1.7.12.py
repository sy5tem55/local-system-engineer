...
description: Safe shell execution for the Local System Engineer (LSE) WSL2/Ubuntu 24.04 agent.
  ...
  call_hermes, hermes_plan, skill_search, skill_record, skill_outcome, task_checkpoint,
  ...
  Changelog:
    ...
    v1.6.4: Hermes Agent subagent — production-ready two-boss architecture.
              ...
              Infrastructure: hermes-gateway.service (hermes-admin, enabled, port
              8642 loopback) + hermes-socat.service (socat proxy, 0.0.0.0:8643 →
              127.0.0.1:8642, auto-starts after hermes-gateway). HERMES_API_URL
              valve default: http://192.168.5.41:8643.
              ...
              Port fix: corrected default URL from :8642 (loopback-only) to :8643.
    ...
    v1.6.2: call_hermes — delegate multi-step tasks to Hermes Agent on node3090.
              ...
              New valves: HERMES_API_URL (http://192.168.5.41:8643), HERMES_API_KEY.
              Port 8643: socat proxy on node3090 (hermes-socat.service) forwards
              0.0.0.0:8643 → 127.0.0.1:8642 (hermes native loopback bind).
...
class Tools:
    ...
    class Valves(BaseModel):
        ...
        HERMES_API_URL: str = Field(
            default="http://192.168.5.41:8643",
    ...
    def task_checkpoint(
        ...
        if not next_prompt.strip() and status == "open":
            return ("CHECKPOINT rejected: next_prompt is required for open tasks — "
        ...
        try:
            ...
            with conn:
                ...
                conn.execute(
                    ...
                    (tid, goal, status, plan, done, findings, unverified,
                     next_prompt, n, created, now),
                )
            conn.close()
            return (f"CHECKPOINT saved: task_id={tid} | status={status} | "
    ...
    def task_resume(self, task_id: str = "") -> str:
        ...
        try:
            ...
            conn.close()
            if not row:
                return ("No matching task block. Either the id is wrong or there is "
                        "no open carried-over work — ask the user what to do next.")
            (tid, goal, status, plan, done, findings, unverified,
             next_prompt, n, created, updated) = row
            return (
                ...
                if _fp_host not in self._device_cache:
                    ...
                    try:
                        _fp_proc = subprocess.run(
                            _fp_cmd, shell=True, capture_output=True, text=True,
                            timeout=15, cwd=cwd,
                        )
        ...
        try:
            ...
            if is_pdf:
                ...
                if text.strip():
                    ...
                    self._fetch_cache[url] = {"text": out, "ts": datetime.now().timestamp()}
            ...
            if ctype and not ("text/html" in ctype or "text/plain" in ctype
            ...
            if text:
                self._fetch_cache[url] = {"text": text, "ts": datetime.now().timestamp()}
                mandate = (f"\n\n[SOURCE-VERIFY MANDATE] Call verify_source_claims(url=\"{url}\", "
                           "claims=\"<fact1>, <fact2>\") before asserting any version number, "
                           "date, or specific value from this source. NOT_FOUND = report as UNVERIFIED.")
    ...
    def verify_source_claims(self, url: str, claims: str) -> str:
        ...
        if cached and ttl > 0 and (datetime.now().timestamp() - cached["ts"]) < ttl:
            ...
            try:
                ...
                return (f"VERIFY ERROR: could not fetch {url}: {e}\n"
        ...
        for claim in claim_list:
            ...
            if idx >= 0:
                ...
                results.append(f"  FOUND    | {claim!r}\n"
                               f"           | excerpt: ...{excerpt}...")
                continue
            ...
            if found_tok:
                tok, exc = found_tok
                results.append(f"  PARTIAL  | {claim!r}\n"
                ...
                ver_ctx = ", ".join(dict.fromkeys(src_vers[:8])) if src_vers else "(none found)"
                results.append(f"  NOT_FOUND| {claim!r}\n"
    ...
    def compact_context(
        ...
        try:
            ...
            try:
                slots_url = (self.valves.LLAMA_SERVER_URL.rstrip("/")
                             + "/slots/0?action=erase")
                kv_req = urllib.request.Request(
                    slots_url, data=b"", method="POST",
                )
        ...
        try:
            ...
            for i, h in enumerate(hits, 1):
                ...
                try:
                    ...
                    _age_d = max(0, (datetime.now().astimezone()
                                     - datetime.fromisoformat(_upd)).days)
    ...
    def index_to_kb(
        ...
        _TIER_CEILING = {
            "ground_truth": 1.0, "primary": 0.8, "secondary": 0.6, "inferred": 0.4,
        }
        ...
        self._log(f"INDEX-KB: title={title} topic={topic} tier={tier} quality={quality_score}")
        ...
        try:
            ...
            return (
                ...
                f"topic={topic} | tier={tier} | quality={quality_score:.2f}{tier_warn}"
            )
        except Exception as e:
    ...
    def skill_search(self, task: str, occupation: str = "", max_results: int = 2) -> str:
        """
        ...
        try:
            ...
            body = {
                "knn": {"field": "embedding", "query_vector": embedding,
                        "k": max_results, "num_candidates": 50, "boost": 0.7},
                "query": {"bool": {
                    "must": [{"multi_match": {"query": task,
                                              "fields": ["task^2", "procedure"],
                                              "boost": 0.3}}],
                    "filter": filters,
                }},
                "_source": ["skill_id", "occupation", "task", "preconditions",
                            "procedure", "verification", "failure_modes",
                            "provenance", "quality", "stats", "pinned"],
                "size": max_results,
            }
            ...
            for i, h in enumerate(hits, 1):
                s = h["_source"]
                try:
                    es.update(index="lse-skills", id=h["_id"], body={"script": {
                        "source": ("ctx._source.stats.uses += 1; "
                                   "ctx._source.stats.last_used = params.now"),
                        "params": {"now": datetime.now().astimezone().isoformat()},
                    }})
                except Exception:
                    pass
                st = s.get("stats", {})
    ...
    def skill_record(self, task: str, occupation: str, procedure: str,
                     verification: str, preconditions: str = "",
                     failure_modes: str = "", provenance: str = "",
                     quality: float = 0.5,
                     source_tier: str = "inferred") -> str:
        """
        ...
        if len(steps) < 2:
            return ("SKILL rejected: procedure has fewer than 2 steps — that is a "
        ...
        try:
            ...
            es = self._es()
            now = datetime.now().astimezone().isoformat()
            dup = es.search(index="lse-skills", body={
                "knn": {"field": "embedding", "query_vector": embedding,
                        "k": 1, "num_candidates": 10},
            ...
            if dup and dup[0]["_score"] >= 0.92:
                ...
                es.update(index="lse-skills", id=ex["_id"], body={"doc": {
                    "task": task, "procedure": steps,
                    "preconditions": _split(preconditions),
                    "failure_modes": _split(failure_modes),
                    "verification": verification,
                    "provenance": _split(provenance),
                    "embedding": embedding, "quality": new_q,
                    "updated_at": now,
                    "version": ex["_source"].get("version", 1) + 1,
                    "archived": False,
                    "source_tier": sk_tier,
                }})
                return (f"SKILL updated: {ex['_source']['skill_id']} | "
            ...
            es.index(index="lse-skills", id=doc_id, document={
                "skill_id": skill_id, "occupation": occupation, "task": task,
                "preconditions": _split(preconditions), "procedure": steps,
                "verification": verification, "failure_modes": _split(failure_modes),
                "provenance": _split(provenance) or ["UNATTRIBUTED"],
                "embedding": embedding, "quality": quality,
    ...
    def skill_outcome(self, skill_id: str, success: bool, evidence: str, source_tier: str = "secondary") -> str:
        """
        ...
        if len(evidence) < ev_min:
            return (f"SKILL outcome rejected: evidence too thin (need >={ev_min} chars "
                    f"for source_tier={so_tier}). Paste the actual verification output "
                    "(command + result), not a claim.")
        try:
            es = self._es()
            resp = es.search(index="lse-skills", body={
                "query": {"term": {"skill_id": skill_id}},
                "_source": ["quality", "stats", "pinned"], "size": 1,
            })
            hits = resp["hits"]["hits"]
            if not hits:
            ...
            now = datetime.now().astimezone().isoformat()
            es.update(index="lse-skills", id=h["_id"], body={"script": {
                "source": ("ctx._source.quality = params.q; "
                ...
                "params": {"q": new_q, "arch": archived, "now": now,
                           "ev": {"ts": now, "success": success,
                                  "evidence": evidence}},
            }})
            tail = " | ARCHIVED (quality floor)" if archived else ""
            return (f"SKILL outcome recorded: {skill_id} | "
    ...
    def pfsense_log_summary(
        ...
        def _ensure_gateway():
            ...
            try:
                ...
                if r.status_code == 200:
                    return True, None
                return False, f"ERROR: Gateway started but /health returned {r.status_code}. Check /opt/local-se/logs/gateway.log"
            except Exception as e:
    ...
    def call_hermes(self, task: str, context: str = "", no_think: bool = True) -> str:
        """
        Delegate a task to the Hermes Agent on node3090 (http://192.168.5.41:8643).
    ...
    def hermes_plan(self, task: str, context: str = "") -> str:
        ...
        corr = hashlib.sha256(
            (task + datetime.now().isoformat()).encode()
        ).hexdigest()[:12]
        request = (
        ...
        if not m:
            return ("PLANNER UNAVAILABLE — no JSON envelope in Hermes reply. "
                    "Proceed with default budgets, checkpoint early.")
        try:
            env = _json.loads(m.group(0))
        except Exception:
            return ("PLANNER UNAVAILABLE — envelope JSON failed to parse. "
                    "Proceed with default budgets, checkpoint early.")
        packaged = str(env.get("packaged_prompt", "")).strip()
        steps = env.get("steps") or []
        if not packaged or not steps:
            return ("PLANNER UNAVAILABLE — envelope missing packaged_prompt "
                    "or steps. Proceed with default budgets, checkpoint early.")
        tid = str(env.get("task_id") or
                  hashlib.sha256(task.encode()).hexdigest()[:8])
        plan_lines = "; ".join(
