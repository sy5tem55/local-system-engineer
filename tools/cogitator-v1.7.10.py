...
class Tools:
    ...
    def skill_record(self, task: str, occupation: str, procedure: str,
        ...
        quality = max(0.2, min(float(quality), 0.7))
    ...
    def skill_outcome(self, skill_id: str, success: bool, evidence: str) -> str:
        """
        ...
        if len(evidence) < 20:
            return ("SKILL outcome rejected: evidence too thin. Paste the actual "
                    "verification output (command + result), not a claim.")
        try:
            es = self._es()
            ...
            new_q = min(1.0, old_q + 0.10) if success else max(0.0, old_q - 0.15)
