...
# `export GOETHE_REDACT_SECRETS=false`) cannot disable redaction
# mid-session.  ON by default — secure default per issue #17691. Users who
# need raw credential values in tool output (e.g. working on the redactor
# itself) can opt out via `security.redact_secrets: false` in config.yaml
# (bridged to this env var in hermes_cli/main.py, gateway/run.py, and
# cli.py) or `GOETHE_REDACT_SECRETS=false` in ~/.hermes/.env. An opt-out
# warning is logged at gateway and CLI startup so operators see the
# downgrade — see `_log_redaction_status()` in gateway/run.py and cli.py.
_REDACT_ENABLED = os.getenv("GOETHE_REDACT_SECRETS", "true").lower() in {"1", "true", "yes", "on"}
...
# LSE ADAPTATION (2026-07-17): CLI credential flags — sshpass -p, --password,
# --token, --api-key, --secret. Ported verbatim from dream_runner._REDACT_RULES
# (in production since 2026-07-12); Hermes has no CLI-flag rule because its
# agent does not shell out with inline credentials. The LSE demonstrably does.
_CLI_CRED_RE = re.compile(
    r"(?i)((?:sshpass\s+(?:-p|--password)|--password|--token|--api-key|--secret)"
    r"[=\s]+)(\"[^\"]+\"|'[^']+'|\S+)"
)
...
def redact_sensitive_text(text: str, *, force: bool = False, code_file: bool = False) -> str:
    ...
    # CLI credential flags (LSE adaptation — see _CLI_CRED_RE above)
    if "-p" in text or "--" in text:
        text = _CLI_CRED_RE.sub(lambda m: m.group(1) + "[REDACTED:cli-credential]", text)
    ...
    # LSE ADAPTATION (2026-07-17): Hermes ships web-URL redaction OFF because
    # its agent must follow magic-link/OAuth URLs mid-flow. The LSE write path
    # is an AUDIT LOG — nothing downstream ever follows a URL from it, and the
    # pfSense api_key-in-query leak (kb/secrets-propagation-report.md P1) is a
    # proven exposure shape here. URL redaction is therefore ON.
    if "://" in text:
