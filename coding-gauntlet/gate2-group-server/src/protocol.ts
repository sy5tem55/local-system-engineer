...
// POST /rooms/:id/agents {handle, endpoint, model?, persona?} -> AgentAccount  (adds a model account to the room)
...
/**
 * The model backend an agent account speaks through. Live impl wraps Gate 1's
 * SSE transport (collect the stream to a string); the acceptance harness injects
 * a deterministic fake. Honour `signal` for the give-up budget.
 */
