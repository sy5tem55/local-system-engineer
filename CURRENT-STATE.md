...
- **Run 8 (2026-07-04) — NEW BASELINE for the llama-ui + goethe_mcp stack: 7/9** (`eval/eval-report-v7.md`, `run_tests(scope="rules")` automated path — 9 rule scenarios only, NOT the full S/A/W/P suite). Subtotals: RESOURCE-AVAILABILITY 2/3, VENDOR-BEHAVIOR 3/3, RELEASE ASSET 2/3. Fingerprint: Goethe (core) v0.3.8, goethe_mcp v1.9.3, model Qwen3.6-27B-UD-Q4_K_XL (ctx 131072, reasoning-budget -1), prompt `node4090-v0.6.0`, 45 tools. Failures RA-2 and RA6 (both refusal/tool-mapping misses — model reached for `execute_command` where a refusal or `fetch_url`/`search_web`/`get_github_release` was expected) filed to the error KB as next-fix candidates.
- **Run 7 (2026-06-04, `eval-report-v6.md`) — 63/63** certified the retired OpenWebUI stack on the full S/A/W/P suite. **NOT comparable to Run 8** — different frontend, different (smaller) suite. Run 8 supersedes it as the current baseline; this is a re-baseline, not a regression.
- **Outstanding:** the full manual S/A/W/P suite has not yet been run against the llama-ui + goethe_mcp stack. `eval/` contains both the skill-pinned `test-suite-v2.md` and newer `test-suite-v3.x` files — which to use is unresolved (flagged in the PH3-3 handoff as a STOP-and-ask case). A full-suite run against this stack is the natural next eval session.
